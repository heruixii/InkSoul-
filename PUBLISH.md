...
### v1.1.5 更新内容
- **前端重构**：清理Vue过渡代码，统一为React；提取公共hooks（useSettings/useCharacters/useLorebooks）；页面组件懒加载优化
- **故事模式导出**：StoryPlay页面新增导出按钮，导出完整故事历史为txt格式
- **对话导出优化**：导出格式从JSON改为易读txt，按角色分行显示

### v1.1.4 更新内容
- **修复数据丢失**：提取脚本在进度文件为空时，从独立文件恢复已有数据，避免用空数据覆盖
- **修复重试模式覆盖**：重试模式仅在提取到新数据时才覆盖输出文件，防止清空已有数据
- **修复文件大小阈值**：从10MB降至100KB，支持更短的小说
- **修复进度加载**：data-loader 正确识别空的 `allCharacters: {}`
