...
/**
 * Vue 3 Composable: 使用版本管理器
 */
export function useVersionManager(options = {}) {
  const versionManager = getVersionManager(options);
  const localVersions = ref(new Map());
  const remoteVersions = ref(new Map());
  const isChecking = ref(false);
  const changedCount = ref(0);

  // 更新本地版本
  const updateLocalVersions = () => {
    localVersions.value = new Map(versionManager.localVersions);
  };

  // 更新远程版本
  const updateRemoteVersions = () => {
    remoteVersions.value = new Map(versionManager.remoteVersions);
  };

  // 检查版本变化
  const checkChanges = async (characterIds = null) => {
    isChecking.value = true;
    try {
      const changes = await versionManager.checkVersionChanges(characterIds);
      changedCount.value = changes.length;
      updateLocalVersions();
      updateRemoteVersions();
      return changes;
    } catch (error) {
      console.error('[useVersionManager] 检查版本变化失败:', error);
      throw error;
    } finally {
  ...
  // 获取角色卡数据
  const getCharacter = async (characterId, forceRefresh = false) => {
    return await versionManager.getCharacter(characterId, forceRefresh);
  };

  // 刷新所有
  const refreshAll = async () => {
    return await versionManager.refreshAll();
  };

  // 监听版本变化
  const onVersionChange = (characterId, callback) => {
    versionManager.onVersionChange(characterId, callback);
  };

  // 移除监听
  const offVersionChange = (characterId, callback) => {
    versionManager.offVersionChange(characterId, callback);
  };

  // 启动定期检查
  const startPeriodicCheck = (interval) => {
    versionManager.startPeriodicCheck(interval);
  };
  ...
  const clearCharacterCache = (characterId) => {
    versionManager.clearCharacterCache(characterId);
  };
